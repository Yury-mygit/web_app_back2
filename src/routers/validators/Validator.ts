...
class Validator {
    ...
    static validateGetAllPays() {
        return [
            body('user_id').exists().withMessage('Validator: user_id is required'),
            body('user_id').isNumeric().toInt().withMessage('Validator: user_id must be a number'),
            body('user_id').custom((value:number) => value > 0 && value < 1000).withMessage('Validator: user_id must be greater than 0 and less than 1000'),
